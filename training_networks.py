...
from scripts.TouchGestureDataset import TouchGestureDataset
...
def training_networks(net, criterion, optimizer, filename, lr = 0.001, epochs = 1000, shuffle_flag = False): 
    ...
    train_dataset = TouchGestureDataset("annotations/annotations_training.csv","./data/training_data", 
                                     transform = transform)
    test_dataset = TouchGestureDataset("annotations/annotations_testing.csv","./data/testing_data", 
                                     transform = transform)
    ...
    for epoch in range(1, n_epochs + 1):   # 데이터셋을 수차례 반복합니다.
        ...
        for idx, data in enumerate(train_dataloader):
            inputs_train = data['dataset'].reshape(-1, 100, net.input_dim).float().to("cuda")
            labels_train = data['label'].to("cuda")
        ...
        for idx, data in enumerate(test_dataloader):
            inputs_test = data['dataset'].reshape(-1, 100, net.input_dim).float().to("cuda")
            labels_test = data['label'].to("cuda")
