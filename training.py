...
net_LSTM = models['LSTM'](input_dim, hidden_dim, layer_dim, output_dim).to("cuda")
optimizer_LSTM = optim.Adam(net_LSTM.parameters())

training_networks(net = net_LSTM, criterion = criterion, optimizer = optimizer_LSTM, 
                  lr = lr, epochs = n_epochs, filename = "LSTM_230129")

#%%
print('RNN network training start!!')
net_RNN = models['RNN'](input_dim, hidden_dim, layer_dim, output_dim).to("cuda")
...
net_GRU = models['GRU'](input_dim, hidden_dim, layer_dim, output_dim).to("cuda")
